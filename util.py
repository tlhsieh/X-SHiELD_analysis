...
def coarse_grain(data4d, fac=1):
    ...
    xquotient = xlen//fac
    yquotient = ylen//fac
    ysouth = (ylen - yquotient*fac)//2
    ynorth = ysouth + yquotient*fac
    
    # helper matrices
    onecol = np.ones((fac, 1))/fac # a column vector
    ones = (onecol,)*xquotient
    right = block_diag(*ones)
    onerow = np.ones((1, fac))/fac # a row vector
